...
const SignUp: React.FC = () => {
    ...
    const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!dispCodeDiv) {
            ...
            try {
                setLoading(true)
                const res = await axios('https://edu-champ-backend.vercel.app/api/auth/register', { method: 'POST', data: { email, password, username, name } })
                if (res.status === 201) {
            ...
            try {
                setLoading(true)
                const res = await axios('https://edu-champ-backend.vercel.app/api/auth/register', { method: 'POST', data: { email, password, enteredVerificationCode, hashedVerificationCode, username, name } })
                if (res.status === 200) {
                    Swal.fire({
                        icon: "success",
                        title: "Valid Code",
            ...
            MySwal.fire({
                ...
                inputValidator: async (value) => {
                    if (!value) {
                        ...
                        try {
                            const res = await axios.post('https://edu-champ-backend.vercel.app/api/editDetails/enterDetails',
                                {
